...
Hidden files in `queue/todo` are ignored. Other regular files are also ignored, but the runner reports their count at the warning level and reports each path when `RUST_LOG=debug` is enabled.
